...
class SeedBatchCodeGenerator implements CodeGeneratorInterface
{
    ...
    public static function generate($seedBatch)
    {
        if (!$seedFarm = $seedBatch->getSeedFarm()) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_seed_farm');
        }
        if (!$seedFarmCode = $seedFarm->getCode()) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_seed_farm_code');
        }
        $variety = $seedBatch->getVariety();
        if (!$variety) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_variety');
        }
        if (!$varietyCode = $variety->getCode()) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_variety_code');
        }
        $species = $variety->getSpecies();
        if (!$species) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_species');
        }
        if (!$speciesCode = $species->getCode()) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_species_code');
        }
        $producer = $seedBatch->getProducer();
        if (!$producer) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_producer');
        }
        if (!$producerCode = $producer->getSeedProducerCode()) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_producer_code');
        }
        $productionYear = (int) $seedBatch->getProductionYear();
        if (!$productionYear) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_production_year');
        }
        // if ($productionYear < 2000 || $productionYear > 2099) {
        if ($productionYear < 1) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.invalid_production_year');
        }
        // TODO: test if year is too far in the future ?

        $batchNumber = $seedBatch->getBatchNumber();
        if (!$batchNumber) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.missing_batch_number');
        }
        // if ($batchNumber < 1 || $batchNumber > 99) {
        if ($batchNumber < 1) {
            throw new InvalidEntityCodeException('sil.seed_batch.error.invalid_batch_number');
        }
